...
export class DisplayInterestsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.activatedRoute.paramMap.subscribe(() => {
      if (window.history.state['user'] !== undefined) {
        this.user.setInitial(window.history.state['user'].id, window.history.state['user'].bio, window.history.state['user'].name, window.history.state['user'].surname);
        this.id = window.history.state['user'].id
      }
      else {
        this.user.setInitial(this.id, "", "", ""); //TODO modifica questo id
      }
    })
    this.getInterests()
      .subscribe(val => {
  ...
  completeUserAccount() {
    ...
    this.userService.completeUserAccount(this.id, this.user)
      .subscribe(val => {
        this.router.navigateByUrl('/user-page', { state: { id: this.id } })
      });
  }
  ...
  back() {
    this.computeActualInterests()
    this.user.setInterests(this.actualInterests);
    this.router.navigateByUrl('/register-second', { state: { id: this.id, user: this.user } })
  }
}
